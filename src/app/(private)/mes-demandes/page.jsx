...
export default function Ecolage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const a = await fetch(
        `http://localhost:3001/panneau/${localStorage.getItem("userId")}`
      );
  ...
  return (
    <div className="flex">
      <div className="w-full h-full flex  gap-3  flex-wrap overflow-y-scroll ">
        {panneau.map((e, i) => (
          <PayLocationCard changeStatut={changeStatut} {...e} key={i} />
        ))}
      </div>
